<?php
    ...
    define('ENCRYPTION_KEY', '1234'); // Use a secure method to store and retrieve this key
    define('ENCRYPTION_METHOD', 'aes-256-cbc'); // Choose your encryption method
    define('IV_LENGTH', openssl_cipher_iv_length(ENCRYPTION_METHOD)); // Get the initialization vector length

    function encryptData($data) {
        $iv = openssl_random_pseudo_bytes(IV_LENGTH); // Generate a secure random IV
        $encryptedData = openssl_encrypt($data, ENCRYPTION_METHOD, ENCRYPTION_KEY, 0, $iv);
        return base64_encode($iv . $encryptedData); // Prepend IV to encrypted data for decryption later
    }
...
if(isset($_POST['request1'])){
    ...
    $currency = 500;
    $user_id = $_SESSION['id'];
    $encryptedCurrency = encryptData($currency); // Encrypt the currency data
    ...
    if($result1){
        ...
        if($requestCurrency==1){
            ...
            $query = "UPDATE user_profile SET requestCurrency = 1, requestedCurrency = '$encryptedCurrency' WHERE id = $user_id";
