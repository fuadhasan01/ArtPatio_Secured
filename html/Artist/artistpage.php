<?php
     ...
     if (!isset($_SESSION['email']) || !hasAccess($_SESSION['category'], 'Artist')) {
         header('Location: ../unauthorized.php'); // Redirect to unauthorized page if access is denied
         exit;
     }

    // Session Timeout Handling
    $timeout_duration = 1800; // Set the session timeout to 30 minutes (1800 seconds)
    if (isset($_SESSION['last_activity']) && (time() - $_SESSION['last_activity']) > $timeout_duration) {
      // If the session has timed out, destroy the session and redirect to login page
      session_unset();
      session_destroy();
       // Start HTML output for the SweetAlert script
       echo "<!DOCTYPE html>
       <html lang='en'>
       <head>
           <meta charset='UTF-8'>
