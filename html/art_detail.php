...
// Prepare and execute the query to get user profile
...
// Prepare and execute the query to get art details
if (isset($_GET['artID'])) {
    $id = intval($_GET['artID']); // Sanitize the input
    $stmt = $conn->prepare("SELECT * FROM arts WHERE artID = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $result = $stmt->get_result();
    $row = $result->fetch_assoc();
    $artname = htmlspecialchars($row['title']);
    $artprice = htmlspecialchars($row['price']);
    $artdescription = htmlspecialchars($row['description']);
    $artimg = htmlspecialchars($row['img']);
    $artID = htmlspecialchars($row['artID']);
    $artistID = htmlspecialchars($row['user_id']);
    $bid = htmlspecialchars($row['initalbid']);
    $artstatus = htmlspecialchars($row['status']);
    $arttype = htmlspecialchars($row['type']);
    $artmaterrial = htmlspecialchars($row['material']);
    $bid_amount = htmlspecialchars($row['bid_amount']);
    $biddingTime = htmlspecialchars($row['bidding_time']);
    $vote = htmlspecialchars($row['votes']);

    // Prepare and execute the query to get artist details
    $stmt = $conn->prepare("SELECT * FROM user_profile WHERE id = ?");
    $stmt->bind_param("i", $artistID);
    $stmt->execute();
    $result = $stmt->get_result();
    $row1 = $result->fetch_assoc();
    $artistname = htmlspecialchars($row1['name']);

    // Prepare and execute the query to get bid details
    $stmt = $conn->prepare("SELECT * FROM bid WHERE art_id = ?");
    $stmt->bind_param("i", $artID);
    $stmt->execute();
    $result = $stmt->get_result();
    if ($result->num_rows < 1) {
        $stmt = $conn->prepare("INSERT INTO bid (art_id, bid_count) VALUES (?, 0)");
        $stmt->bind_param("i", $artID);
        $stmt->execute();
    ...
    $bid_count = htmlspecialchars($row2['bid_count']);
    $bid_user = htmlspecialchars($row2['user_id']);
    $bidprice = $bid + ($bid_count * $bid_amount);

    if ($bid_user) {
        $stmt = $conn->prepare("SELECT * FROM user_profile WHERE id = ?");
        $stmt->bind_param("i", $bid_user);
        $stmt->execute();
        $result = $stmt->get_result();
        $row5 = $result->fetch_assoc();
        $bid_username = htmlspecialchars($row5['name']);
        $user_currancy = htmlspecialchars($row5['currancy']);
    } else {
        $bid_username = NULL;
    }
}
...
<div class="favPopup2">
        ...
        <div style="display: flex; gap: 20px; justify-content: end;">
            <button  class="favYesBtn" id="favYesBtn">Yes</button>
...
<div class="container">
    ...
    <div class="details">
        ...
        <!-- <form action="art_detail.php"method='post'> -->
            <a href="#" class="voteBtn"><button id="voteBtn">+</button></a>
            
            <a href="#" class="buyBtn"><button>Buy</button></a>
            <a href="bid.php?artid=<?php echo $id ?>"><button id="bidBtn">Bid for <?php echo $bidprice ?></button></a>
            
            <a href="#" class="favBtn"><button id="favBtn">Add To Favourite</button></a>
            <a href="#" class="removeBtn"><button id="removeBtn">Remove From Fav</button></a>
            <!-- </form> -->
        </div>
