...
<!-- Ends Nav code -->
  ...
  <div class="container">
    ...
    <?php
    $currandate= date("Y-m-d");
    $curranttime= date("H:i:s");
         
        $query = "SELECT * FROM event WHERE startDate<='$currandate'and endDate>='$currandate' ";
          ...
          if($check_images>0){
           ...
           ?>   
              ...
              <div class="overlay">
                ...
                <a href="eventlogin.php? eventID=<?php echo $row['eventID']  ?>" style="text-decoration:none;"><button>Event Login</button></a>
                </div>
            </div>
          </div>
    <?php
...
<style>@import url('https://fonts.googleapis.com/css2?family=Offside&display=swap');
  *{
